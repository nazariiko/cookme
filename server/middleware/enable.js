...
module.exports = {
  isEnable: async (req, res, next, type, permissionType) => {
    ...
    if (
      type !== "video" &&
      type !== "livestreaming" &&
      req.appSettings[`enable_${type}`] != 1 // Use backticks for template literal
    ) {
      await commonFunctions.getGeneralInfo(req, res, "page_not_found");
      return res.send({ ...req.query, pagenotfound: 1 });
    } else if
      (
        type !== "ads" &&
        req.levelPermissions[`${type}.${permissionType}`] !== 1 &&
        req.levelPermissions[`${type}.${permissionType}`] !== 2
    ) {
      if (!req.user) {
        if (
          permissionType == "view" ||
