...
//     currencies["MNT"] = "Mongolian Tugrug (MNT)"
    ...
    var reg_form = forms.create({
        ...
        payment_appleinapp_method: fields.string({
            ...
            fieldsetClasses:"form_fieldset",
            cssClasses: {"field" : ["form-group"]},
            value:settings.getSetting(req,"payment_appleinapp_method",0).toString()
        }),
        apple_label: fields.string({ 
            widget: formFunctions.makeClickable({content : 'Apple In-App Purchase will work on iOS app only.',replace: []}),
            cssClasses:{"field" : ["form-group","form-description"]},
        }),
        apple_inapp_purchasesecret: fields.string({
            ...
            cssClasses: {"field" : ["form-group"]},
            widget: widgets.text({"classes":["form-control"]}),
            value:req.loguserallowed ? "****" : settings.getSetting(req,"apple_inapp_purchasesecret",'')
        }),
        apple_webhook_label: fields.string({ 
            widget: widgets.label({content : 'Apple Store Server Notification URL: '+process.env.PUBLIC_URL+"/apple-pay-notification" }),
            cssClasses:{"field" : ["form-group","form-description"]},
        }),
...
exports.approveBank = async(req,res) => {
    ...
    await globalModel.custom(req, "SELECT * from bankdetails LEFT JOIN users ON bankdetails.owner_id = users.user_id LEFT JOIN userdetails ON userdetails.user_id = users.user_id where bankdetails.bank_id = ?", id).then(async result => {
        if (result && result.length) {
            ...
            if(bank_details && bank_details.type == "video_purchase"){
                ...
                await globalModel.create(req, {type:"video_purchase",id:video.video_id, owner_id: bank_details.owner_id, package_id: 0, status: "completed",creation_date: currentDate, modified_date: currentDate, gateway_profile_id: "Bank Transfer",order_id:req.session.orderId }, "subscriptions").then(async result => {
                    ...
                    await videoPurchaseModel.insertTransaction(req, {gateway_id: 3 , order_id:req.session.orderId,admin_commission:commission_amount, gateway_transaction_id: "Bank Transfer", owner_id: video.owner_id ,sender_id:bank_details.owner_id, state: "completed", price: parseFloat(video.price).toFixed(2) - commission_amount, currency: req.appSettings.payment_default_currency, creation_date: currentDate, modified_date: currentDate,id:video.video_id,type:"video_purchase" }).then(async result => {
                        ...
                        if(parseInt(video.type) == 10 || parseInt(video.type) == 11){
                            //buyer
                            notifications.insert(req, {owner_id:bank_details.owner_id,insert:true, type: "livestream_purchased", subject_type: "users", subject_id: bank_details.owner_id, object_type: "videos", object_id: video.video_id,forceInsert:true }).then(result => {
                            ...
                            //owner
                            notifications.insert(req, {notChangeOwnerID:true,owner_id:video.owner_id,insert:true, type: "livestream_purchased_owner", subject_type: "users", subject_id: bank_details.owner_id, object_type: "videos", object_id: video.video_id,forceInsert:true }).then(result => {
                            ...
                            //buyer
                            notifications.insert(req, {owner_id:bank_details.owner_id,insert:true, type: "video_purchased", subject_type: "users", subject_id: bank_details.owner_id, object_type: "videos", object_id: video.video_id,forceInsert:true }).then(result => {

                            }).catch(err => {
                                console.log(err)
                            })
                            //owner
                            notifications.insert(req, {notChangeOwnerID:true,owner_id:video.owner_id,insert:true, type: "video_purchased_owner", subject_type: "users", subject_id: bank_details.owner_id, object_type: "videos", object_id: video.video_id,forceInsert:true }).then(result => {
            ...
            }else if(bank_details && (bank_details.type == "purchase_series_purchase" ||  bank_details.type == "purchase_movie_purchase" || bank_details.type == "rent_series_purchase" || bank_details.type == "rent_movie_purchase" )){
                type = bank_details.type+"_approved";

                let movie = {}
                await movieModel.findByCustomUrl(bank_details.resource_id,req).then(result => {
                    if(result){
                        movie = result
                    }
                })
                ...
                let type = bank_details.type == "purchase_movie_purchase" || bank_details.type == "purchase_series_purchase" ? "purchase" : "rent"
                ...
                await movieModel.checkMoviePurchased({id:movie.movie_id,owner_id:bank_details.owner_id,type:movie.category},req).then(result => {
                    if(result){
                        isValid = false
                    }
                }).catch(err => {
                ...
                await globalModel.create(req, {owner_id:bank_details.owner_id,gateway_id:3,state:"initial",creation_date:currentDate,source_type:movieSeriesType,source_id:movie.movie_id}, "orders").then(result => {
                    if (result) {
                ...
                await globalModel.create(req, {type:movieSeriesType,id:movie.movie_id, owner_id: movie.owner_id, package_id: 0, status: "completed",creation_date: currentDate, modified_date: currentDate, gateway_profile_id: "Bank Transfer",order_id:req.session.orderId }, "subscriptions").then(async result => {
                    const moviePurchaseModel = require("../../models/videoPurchase")
                    await moviePurchaseModel.insertTransaction(req, {gateway_id: 3 , order_id:req.session.orderId,admin_commission:commission_amount, gateway_transaction_id: "Bank Transfer", owner_id: movie.owner_id ,sender_id:req.session.movie_user_id, state: "completed", price: parseFloat(price).toFixed(2) - commission_amount, currency: req.appSettings.payment_default_currency, creation_date: currentDate, modified_date: currentDate,id:movie.movie_id,type:movieSeriesType },true).then(async result => {
                        //update user balance
                        ...
                        //buyer
                        notifications.insert(req, {owner_id:bank_details.owner_id,insert:true, type: typePurchased, subject_type: "users", subject_id: bank_details.owner_id, object_type: "movies", object_id: movie.movie_id,forceInsert:true }).then(result => {

                        }).catch(err => {
                            console.log(err)
                        })
                        //owner
                        notifications.insert(req, {notChangeOwnerID:true,owner_id:movie.owner_id,insert:true, type: typeOwnerPurchased, subject_type: "users", subject_id: bank_details.owner_id, object_type: "movies", object_id: movie.movie_id,forceInsert:true }).then(result => {
                        ...
                        globalModel.update(req,{gateway_transaction_id:"Bank Transfer",state:"completed",'source_id':movie.movie_id},"orders","order_id",req.session.orderId)
                    })
                })
            }
