...
exports.addTranslations = async(req,res) => {
    ...
    if(!isValid){
        ...
        fs.writeFile(req.serverDirectoryPath + "/../public/locales/" + code + "/common.json", data, 'utf8', function (err) {
...
exports.getGoogleTranslationData = (code,text,req) => {
    return new Promise(function(resolve, reject) {
        ...
        var config = {
            method: 'POST',
            url: `https://www.googleapis.com/language/translate/v2?key=${APIkey}&source=en&target=${code}&q=${encodeURIComponent(text)}`,
            headers: { 
...
exports.getTranslationData = (code,req) => {
    return new Promise(function(resolve, reject) {
        const filePath = req.serverDirectoryPath + "/../public/locales/" + code + "/common.json"
...
exports.delete = async (req, res) => {
    ...
    globalModel.delete(req, "languages", "language_id", language_id).then(result => {
        deleteFolderRecursive(req.serverDirectoryPath + "/../public/locales/" + existingLanguage.code)
...
exports.download = async (req,res,next) => {
    ...
    var file = req.serverDirectoryPath + "/../public/locales/" + code + "/common.json";
    ...
    if(req.imageError){
        ...
        fs.readFile(req.serverDirectoryPath+"/public/"+imagename,{ encoding: 'utf8' },function(err,data) {
            if(err){
                ...
                fs.writeFile(req.serverDirectoryPath + "/../public/locales/" + languageCode + "/common.json", data, 'utf8', function (err) {
                    if (err) {
                        res.status(200).send({error:1,message:"Error in uploading file.",'code':languageCode})
                    }else{
    ...
    fs.readFile(filePath, { encoding: 'utf8' }, function (err, data) {
        if (!err) {
            ...
            if (req.editPhrase || req.addPhrase) {
                ...
                if (valid) {
                    fs.writeFile(req.serverDirectoryPath + "/../public/locales/" + languageCode + "/common.json", JSON.stringify(fileJsonDataOrg,null,4), 'utf8', function (err) {
                        if (err) {
                            console.log(err);
                        }
                    });
            ...
            if (!language_id) {
                        ...
                        globalModel.custom(req, "SELECT code FROM languages where `default` = ?", ["1"]).then(results => {
                            if (results) {
                                ...
                                if (defaultCode[0]) {
                                    ...
                                    fs.mkdir(req.serverDirectoryPath + "/../public/locales/" + form.data.code, "0777", function (error) {
                                        if (!error) {
                                            fs.copyFile(req.serverDirectoryPath + "/../public/locales/" + code + "/common.json", req.serverDirectoryPath + "/../public/locales/" + form.data.code + "/common.json", (err) => {
                                                if (err) throw err;
                                            });
                                        }
                                    })
                ...
                globalModel.custom(req, "SELECT * FROM languages where `language_id` = ?", [language_id]).then(codeData => {
                    ...
                    if (form.data.code != codeDataRes.code) {
                        ...
                        fs.rename(req.serverDirectoryPath + "/../public/locales/" + codeDataRes.code, req.serverDirectoryPath + "/../public/locales/" + form.data.code, err => {
