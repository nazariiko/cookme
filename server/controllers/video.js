...
exports.successul = async (req, res, next) => {
    ...
    if(gateway == "2" && stripeToken){
        ...
        adsTransaction.reduceOwnerWalletAmount(req,{owner_id:req.user.user_id,amount:parseFloat(video.price - commission_amount).toFixed()})
...
exports.adClicked = async(req,res) => {
        ...
        await globalModel.custom(req,"SELECT * FROM advertisements_admin WHERE ad_id = ?",[id]).then(result => {
            if(result){
                ...
                if(adsResults && adsResults.length){
                    ...
                    valid = true
                    let adUrl = ad.click_link.indexOf("http://") == 0 || ad.click_link.indexOf("https://") == 0 ? "http://"+ad.click_link : ad.click_link
                    return res.send({redirect:true,url:decodeURI(adUrl)});
                }
            } 
        })
    }else if(type == "user"){
    ...
    if(!valid){
        res.send({redirect:true,url:process.env.PUBLIC_URL});
    }
}
