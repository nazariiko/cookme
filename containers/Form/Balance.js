...
const Balance = (props) => {
    ...
    const onSubmit = model => {
        ...
        let userAs = props.pageData.user ? `?user=${props.pageData.user}` : "";
        let price = props.pageData.levelPermissions['member.monetization_threshold_amount']
        if(state.monetization_threshold_amount){
            price = state.monetization_threshold_amount
        }
        if(parseFloat(state.member.balance) >= parseFloat(model['amount']) && parseFloat(price) <= parseFloat(model['amount'])){
            ...
            if(parseFloat(state.member.balance) < parseFloat(model['amount'])){
                setState({ error: [{"field":"amount","message":"Enter amount must be less than available balance."}], submitting: false });
            }else if( parseFloat(price) > parseFloat(model['amount'])){
                setState({ error: [{"field":"amount","message":"Enter amount must be greater than minimum withdraw amount."}], submitting: false });
            }
        }
    };
    const recharge = (e) => {
    ...
    const walletFormSubmit = (e) => {
        ...
        setState({ adsWallet: false,gatewaysURL:"/ads/recharge?fromBalance=1&amount=" + encodeURI(state.walletAmount),gateways:true })
    ...
    const walletValue = (e) => {
        ...
        let userBalance = {}
         userBalance['package'] = { price: parseFloat(state.member.balance ? state.member.balance : 0) }  
        let priceData = Currency({...props,...perclick}).replace("<!-- -->","")
        let formFields = [
            { key: "wallet", label: "Wallet Total",props: { readOnly: true }, value: Currency({...props,...wallet}).replace("<!-- -->","") },
            { key: "balance", label: "Available Balance",props: { readOnly: true }, value: Currency({...props,...userBalance}).replace("<!-- -->","") },
            { key: "paypal_email", label: "Paypal Email", value: state.member.paypal_email ? state.member.paypal_email : "",isRequired:true },
            { key: "amount", label: "Amount", type: "text", value: "", placeholder:"00.00",isRequired:true },
            {
                key: "res_type_1",
                type: "content",
                content: '<h6 class="custom-control minimum_amount_cnt">'+props.t("Minimum withdraw amount should be greater than or equal to {{data}}.",{data:`(${priceData})`})+'</h6>'
            }
        ]
        ...
        if(state.gateways){
            ...
            }} bank_price={state.walletAmount} bank_type="recharge_wallet" bank_resource_type="user" bank_resource_id={props.pageData.loggedInUserDetails.username} tokenURL={`ads/successulPayment?fromBalance=1&amount=${encodeURI(state.walletAmount)}`} closePopup={() => setState({gateways:false})} gatewaysUrl={state.gatewaysURL} />
        }
        ...
        return (
            <React.Fragment>
                ...
                <button className="custom-control open_withdraw floatR" href="#">{props.t("Withdrawal Requests")}</button>
                <button className="floatR balance_recharge" onClick={(e) => recharge(e)}>{Translate(props, "Recharge Wallet")}</button>
                <div ref={myRef}>
                <Form
