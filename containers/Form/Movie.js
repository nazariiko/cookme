    ...
    const onSubmit = model => {
        ...
        if(state.movie_sell && props.pageData.appSettings['movie_commission_type']  == 1 && props.pageData.appSettings['movie_commission_value'] > 0){
            if(model['price'] && parseFloat(model['price']) > 0){
                if(model['price'] <=  props.pageData.appSettings['movie_commission_value']){
                    let perprice = {}
        ...
        if(state.movie_rent && props.pageData.appSettings['movie_commission_rent_type']  == 1 && props.pageData.appSettings['movie_commission_rent_value'] > 0){
            if(model['rent_price'] && parseFloat(model['rent_price']) > 0){
                if(model['rent_price'] <=  props.pageData.appSettings['movie_commission_rent_value']){
                    let perprice = {}
    ...
    let fUrl = props.router.asPath.split("?");
        ...
        if(state.movie_sell){
            ...
            if(props.pageData.appSettings['movie_commission_type']  == 1 && props.pageData.appSettings['movie_commission_value'] > 0){
                let perprice = {}
                perprice['package'] = { price: props.pageData.appSettings['movie_commission_value'] }
                postDescription = '<div class="form-post-description">' + props.t("Price enter must be greater than {{price}}.",{price:Currency({...props,...perprice}).replace("<!-- -->","")}) + '</div>'
            }
            groupData0.push({"postDescription":postDescription, key: "price", label: state.selectType == "movie" ? "Price (Put 0 for free movies)" :  "Price (Put 0 for free series)", value: state.editItem ? state.editItem.price : null,isRequired:true })
        }
        ...
        if(state.movie_rent){
            ...
            if(props.pageData.appSettings['movie_commission_rent_type']  == 1 && props.pageData.appSettings['movie_commission_rent_value'] > 0){
                let perprice = {}
                perprice['package'] = { price: props.pageData.appSettings['movie_commission_rent_value'] }
                postDescriptionData = '<div class="form-post-description">' + props.t("Rent Price enter must be greater than {{price}}.",{price:Currency({...props,...perprice}).replace("<!-- -->","")}) + '</div>'
            }
            groupData0.push({"postDescription":postDescriptionData, key: "rent_price", label: state.selectType == "movie" ? "Rent Price (Put 0 to disable rent movies)" : "Rent Price (Put 0 to disable rent series)", value: state.editItem ? state.editItem.rent_price : null,isRequired:true })
        }
