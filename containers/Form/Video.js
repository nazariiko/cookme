    ...
    const onSubmit = model => {
        ...
        if(state.sell_videos && props.pageData.appSettings['video_commission_type']  == 1 && props.pageData.appSettings['video_commission_value'] > 0){
            if(model['price'] && parseFloat(model['price']) > 0){
                if(model['price'] <=  props.pageData.appSettings['video_commission_value']){
                    let perprice = {}
    ...
    const saveTips = (e) => {
        ...
        tips.forEach((item,index) => {
            if(parseFloat(item.amount) > 0){
                if(parseFloat(props.pageData.appSettings[state.videoAmountType+'_commission_value']) > 0 && parseInt(props.pageData.appSettings[state.videoAmountType+'_commission_type']) == 1 && parseFloat(item.amount) <= parseFloat(props.pageData.appSettings[state.videoAmountType+'_commission_value'])){
                    valid = false
                ...
                item.error = props.t("Enter amount must be greater than {{price}}.",{price : parseFloat(props.pageData.appSettings[state.videoAmountType+'_commission_value']) > 0 && parseInt(props.pageData.appSettings[state.videoAmountType+'_commission_type']) == 1 ? Currency({...props,...perprice}) : 0});
                valid = false;
            }
        })
        ...
        if(state.chooseType == "upload" && state.sell_videos && (!state.editItem || state.editItem.type == 3)){
            ...
            formFields.push({ key: "price", label: "Price (Leave empty for free videos)", value: state.editItem ? state.editItem.price : null,isRequired:true })
