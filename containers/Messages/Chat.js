...
const Chat = (props) => {
    ...
    useEffect(() => {
        ...
        props.socket.on('chatMessageCreate', data => {
            ...
            if(parseInt(chat.message_id) == parseInt(stateRef.current.id)){
                props.socket.emit('readChat', {id:props.pageData.loggedInUserDetails.user_id,message_id:stateRef.current.id})
                if(parseInt(chat.chat_user_id) != parseInt(props.pageData.loggedInUserDetails.user_id)){
                    chat.is_read = 1;
                    chat.seen = 1;
                }
    ...
    const onEmojiClick = (data) => {
        ...
        setTimeout(() => {
            ...
            textareaInput.current.selectionEnd = textareaInput.current.selectionStart =  selectionEnd + 2
        },200)
    }
    ...
    const loadMoreData = (type) => {
        ...
        if(type == "new"){
            props.socket.emit('readChat', {id:props.pageData.loggedInUserDetails.user_id,message_id:state.id})
        }
        if(!type)
        ...
        axios.post(url, formData, config)
            .then(response => {
                if (response.data.chats) {
                    ...
                    setState({ page: state.page + 1, pagging: pagging, chats: chats, loading: false })
                    setTimeout(() => {
                        let _ = this
    ...
    const submitForm = () => {
        if(state.selectedFile || state.textValue){
            ...
            axios.post(url, formData, config)
                .then(response => {
                    if(response){
                        setState({textValue:"",showEmoji:false,selectedFile:null})
                    }
                })
        }
    }
    const clearChat = () => {
        let message = props.t("Are you sure that you want to delete the conversation?")
        swal({
                ...
                if (willDelete) {
                    ...
                    axios.post(url, formData)
                        .then(response => {
                            ...
                            swal("Error", props.t("Something went wrong, please try again later"), "error");
                        });
                    //delete
                } else {
        ...
        let type = "m"
        if(state.message && state.message.resource_id != props.pageData.loggedInUserDetails.user_id){
            type = "r"
        }
        ...
        return (
            <div style={{...style}} className={`${props.fromSmallChat ? "w-100 overflow-hidden chatBoxWrap" : "w-100 overflow-hidden position-relative"}${state.minimize == 1 ? " minimize" : ""}`}>
                <div className="p-3 p-lg-4 border-bottom user-chat-topbar">
                    <div className="row align-items-center">
                        <div className="col-sm-12 col-12">
                            <div className="d-flex align-items-center flex-wrap">
                                <div className="me-3 ms-0 ml5">
                                    <Link href="/member" customParam={`id=${state.message[`${type}username`]}`} as={`/${state.message[`${type}username`]}`}>
                                        <a>
                                            <img src={props.pageData.imageSuffix+state.message[`${type}avtar`]} className="rounded-circle avatar-xs" alt={state.message[`${type}displayname`]} />
                                        </a>
                                    </Link>
                                </div>
                                            ...
                                            <a className="text-reset user-profile-show">
                                                {state.message[`${type}displayname`]}
                                                {
                                                    parseInt(state.message[`${type}verified`]) == 1 ? 
                                                        <span className="verifiedUser" title={props.t("verified")}><span className="material-icons" data-icon="check"></span></span>
                                                    : null
                                                }
                                            </a>
                                        </Link>
                ...
                <div className="chat-conversation p-3 p-lg-4 sidebar-scroll" ref={scrollDiv} onScroll={onMainScroll}>
                    <div>
                            ...
                            }
                            {
                                state.chats.length > 0 ? 
                                    state.chats.map(chat => {
                                        return (
                                            <li key={chat.messages_text_id} className={`${parseInt(chat.chat_user_id) == props.pageData.loggedInUserDetails.user_id ? "right" : ""}`}>
                                                <div className="conversation-list">
                                                    <div className="chat-avatar">
                                                        <img src={props.pageData.imageSuffix+chat.avtar}
                                                            ...
                                                            
                                                            {
                                                                                ...
                                                                                <li
                                                                                    ...
                                                                                    <div>
                                                                                        <a className="popup-img d-inline-block m-1"
                                                                                            href="#" onClick={(e) => {openImage(props.pageData.imageSuffix+chat.image,e)}}>
                                                                                            <img src={props.pageData.imageSuffix+chat.image}
                                                                ...
                                                                <p className="chat-time mb-0">
                                                                    <span className="align-middle">
                                                                        <Timeago {...props}>{chat.creation_date}</Timeago>
                                                                    </span>
                                                                </p>
                                                            </div>
                                                            {
                                                                chat.chat_user_id == props.pageData.loggedInUserDetails.user_id ? 
                    ...
                    <div className="row g-0">
                        ...
                        <div className="preview chatPreviewImg">
                            <div className="previewUploadImg">
                                <div>
                                    <img src={state.selectedFile ? (typeof state.selectedFile == "string" ? state.selectedFile : createObjectURL(state.selectedFile)) : null} alt={props.t("Image Preview")} />
                        ...
                        : state.selectedFile && state.selectedFileType == "video" ? 
                            <div className="preview">
                                <div className="previewUploadImg">
                                    <div>
                                    <video width="400" controls className="rounded border">
                                        <source src={createObjectURL(state.selectedFile)} type="video/mp4" />
                                        {props.t("Your browser does not support HTML video.")}
                                    </video>
                                        <span className="close closePreviewImage" onClick={(e) => {
                                            setState({selectedFile:null})
                        ...
                        :null
                            ...
                            <div className="chat-input-links ms-md-2 me-md-0">
                                <ul className="list-inline mb-0">
                                    <li className="list-inline-item emojiBtn" ref={emojiContainer}>
                                        {
                                            state.showEmoji ? 
                                    ...
                                    <li className="list-inline-item attachBtn" data-bs-toggle="tooltip"
                                        data-bs-placement="top" title={props.t("Attached image")} >
                                        <button type="button"
                                            onClick={(e) => {
                                            ...
                                            <input type="file" style={{display:"none"}} onChange={selectedFile} name="file" accept="image/*,video/mp4" ref={inputFile} />
                                        </button>
                                    </li>
