...
const Index = (props) => {
    ...
    useEffect(() => {
        ...
        }else if(reduxstate.search.searchChanged){
            const fieldsValues = {}
            fieldsValues.h = reduxstate.search.searchValue
            setState({
                submitting: false,
                items: [],
                pagging: false,
    ...
    useEffect(() => {
        if(props.pageData.appSettings["fixed_header"] == 1 && props.hideSmallMenu && !reduxstate.search.menuOpen){
    ...
    useEffect(() => {
        if(reduxstate.search.searchChanged){
            props.setSearchChanged(false)
            submitForm()
        }
    },[reduxstate.search.searchChanged])
