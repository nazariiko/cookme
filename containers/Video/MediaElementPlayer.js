...
const MediaElementPlayer = (props) => {
  ...
  const initiatePlayer = () => {
    ...
    if (state.adminAdVideo) {
      ...
      adVideoLink = props.imageSuffix + state.userAdVideo.media;
    }
    if (
      state.adminAdVideo &&
  ...
  if (state.video.status == 1 && resolutionsVideo.length > 0) {
    ...
    if (state.video.type == 1) {
      url = `https://www.youtube.com/watch?v=${props.code}`;
      type = "video/youtube";
    } else if (state.video.type == 2) {
      url = `https://vimeo.com/${props.code}`;
      type = "video/vimeo";
    } else if (state.video.type == 4) {
      url = `https://www.dailymotion.com/video/${props.code}`;
