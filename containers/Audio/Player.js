...
const Player = (props) => {
    ...
    const [state, setState] = useReducer(
        ...
        {
            audios:reduxState.audio.audios,
            width:props.isMobile ? props.isMobile : 993,
            song_id:reduxState.audio.song_id,
            minimizePlayer:false,
            pausesong_id:reduxState.audio.pausesong_id,
            currentTime: null,
            playCount:[],
    ...
    const getItemIndex = (item_id) => {
        const audios = [...reduxState.audio.audios];
        const itemIndex = audios.findIndex(p => p["audio_id"] == item_id);
        return itemIndex;
    }
    
    const ended = () => {
        let song_id = reduxState.audio.song_id
        let itemIndex = 0
        itemIndex = getItemIndex(song_id)
        if (itemIndex > -1) {
            const items = [...reduxState.audio.audios]
            if(itemIndex+2 <= reduxState.audio.audios.length){
                itemIndex = itemIndex + 1
            }else{
                itemIndex = 0
            }
            props.updateAudioData({audios:reduxState.audio.audios, song_id:items[itemIndex].audio_id,pausesong_id:0})
        }
    }
    ...
    const previous = () => {
        ...
        if (itemIndex > -1) {
            const items = [...reduxState.audio.audios]
            if(itemIndex == 0){
                itemIndex = reduxState.audio.audios.length - 1
            }else{
                itemIndex = itemIndex - 1
            }
            props.updateAudioData({audios:reduxState.audio.audios, song_id:items[itemIndex].audio_id,pausesong_id:0})
    ...
    const playSong = () => {
        props.updateAudioData({audios:reduxState.audio.audios, song_id:reduxState.audio.song_id,pausesong_id:0})
        audioTag.current.play();
    }
    const pauseSong = () => {
        props.updateAudioData({audios:reduxState.audio.audios, song_id:reduxState.audio.song_id,pausesong_id:reduxState.audio.song_id})
    ...
    const playStart = () => {
        let currentPlayingSong = getItemIndex(reduxState.audio.song_id)
        let audio = reduxState.audio.audios[currentPlayingSong];
        let sessionPassword = props.pageData && props.pageData.audioPassword ? props.pageData.audioPassword : []
        if(audio.view_privacy == "password" && sessionPassword.indexOf(reduxState.audio.song_id) == -1 && !audio.passwords && ( !props.pageData.levelPermissions || props.pageData.levelPermissions["audio.view"] != 2 ) 
        ...
        if(state.playCount[reduxState.audio.song_id]){
            return
        }
        let counts = [...state.playCount]
        counts[reduxState.audio.song_id] = reduxState.audio.song_id
        setState({playCount:counts})
        const formData = new FormData()
        formData.append('id', reduxState.audio.song_id)
        ...
        axios.post(url, formData, config)
            .then(response => {
                if (response.data.error) {
                    ...
                    passwords[reduxState.audio.song_id] = reduxState.audio.song_id
    ...
    const audioSeekSet = (e) => {
        ...
        if(!reduxState.audio.audios || (!reduxState.audio.audios.length && !reduxState.audio.audios.length && !reduxState.audio.song_id)){
            return null
        }
        let currentPlayingSong = getItemIndex(reduxState.audio.song_id)
        let audio = reduxState.audio.audios[currentPlayingSong];
        ...
        if(reduxState.audio.song_id == reduxState.audio.pausesong_id){
            try{
                audioTag.current.pause();
            }catch(err){
        ...
        return (
            ...
            <div className="playbar">
                ...
                <ul className="controller">
                    {
                        reduxState.audio.audios && reduxState.audio.audios.length > 1 ? 
                            <li onClick={previous}>
                    ...
                    <li>
                    {
                        reduxState.audio.pausesong_id == audio.audio_id ?
                    ...
                    {
                        reduxState.audio.audios.length > 1 ? 
                    <li onClick={ended}>
