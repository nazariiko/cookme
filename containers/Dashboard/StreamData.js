    ...
    const onSubmit = model => {
        ...
        if(props.pageData.appSettings['livestreaming_commission_type']  == 1 && props.pageData.appSettings['livestreaming_commission_value'] > 0){
            if(model['price'] && parseFloat(model['price']) > 0){
                if(model['price'] <=  props.pageData.appSettings['livestreaming_commission_value']){
                    let perprice = {}
    ...
    const saveTips = (e) => {
        ...
        tips.forEach((item,index) => {
            if(parseFloat(item.amount) > 0){
                if(parseFloat(props.pageData.appSettings['videotip_commission_value']) > 0 && parseInt(props.pageData.appSettings['videotip_commission_type']) == 1 && parseFloat(item.amount) <= parseFloat(props.pageData.appSettings['videotip_commission_value'])){
                    valid = false
