...
const Menu = (props) => {
    ...
    useEffect(() => {
        if (props.pageData.searchTitleText && props.pageData.searchTitleText != state.search) {
            setState({ search: props.pageData.searchTitleText ? props.pageData.searchTitleText : "", style: false })
        }
    },[props])
        ...
        props.socket.on('notifications', data => {
            if (props.pageData && props.pageData.loggedInUserDetails) {
                if (data.owner_id == props.pageData.loggedInUserDetails.user_id) {
                    setState({ notifications: [data.notification, ...stateRef.current.notifications], unread: stateRef.current.unread + 1 })
                }
            }
        })
    ...
    const openToggle = (type, e) => {
        ...
        if (type == "notifications") {
            ...
            setState({ style: style == "block" ? "none" : "block", type: style == "none" ? "notifications" : "" });
            if (props.mobileMenu) {
            ...
            setState({ style: style == "block" ? "none" : "block", type: style == "none" ? "settings" : "" });
            if (props.mobileMenu) {
                if (state.style == "block")
                    $("body").addClass("menu_open");
                else
            ...
            setState({ style: style == "block" ? "none" : "block", type: style == "none" ? "createbuttons" : "" });
            if (props.mobileMenu) {
                if (state.style == "block")
                    $("body").addClass("menu_open");
                else
            ...
            setState({ style: style == "block" ? "none" : "block", type: style == "none" ? "search" : "" });
    ...
    const handleClickOutside = (e) => {
        let style = "block"
        if (stateRef.current.type && e.target && !$(e.target).hasClass("parent") && (!$(e.target).data("bs-toggle") || ($(e.target).data("bs-toggle") && $(e.target).data("bs-toggle") != "dropdown"))) {
            if (stateRef.current.type == "notifications") {
                if (dropNotificationRef.current && !dropNotificationRef.current.contains(e.target)) {
                ...
                if (dropSettingsRef.current && !dropSettingsRef.current.contains(e.target)) {
        ...
        } else if (e.target && !$(e.target).hasClass("menu-bar") && !$(e.target).hasClass("dropdown-toggle") && (!$(e.target).data("bs-toggle") || ($(e.target).data("bs-toggle") && $(e.target).data("bs-toggle") != "dropdown")) && (!dropMenuButtons.current || !dropMenuButtons.current.contains(e.target))) {
    ...
    const updateOpenedMenu = (type) => {
        setState({ opendMenu: type })
    }
    const markAllRead = () => {
    ...
    const loadMoreContent = (firstLoaded) => {
        ...
        if (!firstLoaded) {
            formData.append('id', stateRef.current.notifications[stateRef.current.notifications.length - 1].notification_id)
        }
        axios.post(url, formData)
            .then(response => {
                if (response.data.error) {
                    ...
                    setState({ loading: false, notifications: [...stateRef.current.notifications, ...response.data.notifications], unread: response.data.unread, pagging: response.data.pagging })
                }
            }).catch(err => {
                setState({ loading: false })
            });
    ...
    const openMobileMenu = (e) => {
            ...
            menus = props.pageData.menus.menus.map(elem => {
                return (
                    <li className={"nav-item" + (elem.submenus ? " dropdown dropdownmenu" : "")} key={elem.menu_id}>
                        <Link href={elem.params ? elem.params : elem.url} customParam={elem.customParam} as={elem.url} prefetch="false">
                                        ...
                                        elem.submenus.map(subMenu => {
                                            return (
                                                <li className={"nav-item" + (subMenu.subsubmenus ? " dropdown dropdownmenu" : "")} key={subMenu.menu_id}>
                                                    <Link href={subMenu.params ? subMenu.params : subMenu.url} customParam={subMenu.customParam} as={subMenu.url} prefetch="false">
                                                                    ...
                                                                    subMenu.subsubmenus.map(subsubMenu => {
                                                                        return (
                                                                            <li key={subsubMenu.menu_id}>
                                                                                <Link href={subsubMenu.params ? subsubMenu.params : subsubMenu.url} customParam={subsubMenu.customParam} as={subsubMenu.url} prefetch="false">
                    ...
                    <li key={elem.menu_id}>
                        {
                            !elem.submenus ?
                                <Link href={elem.params ? elem.params : elem.url} customParam={elem.customParam} as={elem.url}>
                                    <a className={"nav-link" + (elem.submenus ? " dropdown-toggle" : "")} {...attribute} target={elem.target} id={"navbarDropdown" + elem.menu_id} prefetch="false"
                                    >
                                        {props.t(elem.label)}
                                    </a>
                                </Link>
                                                            ...
                                                            <ul className="collapse list-unstyled MobMenuSidebarLvl2" id={"navbarDropdown" + subMenu.menu_id}>
                                                                {
                                                                    subMenu.subsubmenus.map(subsubMenu => {
                                                                        return (
                                                                            <li key={subsubMenu.menu_id}>
                                                                                <Link href={subsubMenu.params ? subsubMenu.params : subsubMenu.url} customParam={subsubMenu.customParam} as={subsubMenu.url} prefetch="false">
                                                                                    <a target={subsubMenu.target} className="dropdown-item">{props.t(subsubMenu.label)}</a>
                                                                                </Link>
                                                                            </li>
                                                                        )
                                                                    })
        ...
        let logo = '';
        if (props.pageData.themeMode === 'dark') { // Fixed: Changed from '==' to '===' for strict equality
            logo =
                ImageUrl +
                props.pageData.appSettings['darktheme_logo'] +
                '?format=auto&width=640/340'; // Fixed: Replaced '/' with '+' for URL concatenation
        } else {
            logo =
                ImageUrl +
                            ...
                            <div className="col-lg-12">
                                ...
                                <div className="logo">
                                        <Link href="/" prefetch="false">
                                            <a><img src={logo} alt="AI Cooking"/></a>
                                        </Link>
                                    </div>
                                    ...
                                    <div className="searchBoxWrap">
                                        <div className="searchBoxContent">
                                            <form action="#" onSubmit={search.bind(this)}>
                                                <input type="text" value={state.search} onChange={(e) => { setState({ search: e.target.value }) }} />
                                                <button type="submit" id="Search" aria-labelledby={Translate(props, "Search")} >
                                    ...
                                    <div className="rightTop">
                                    {
                                            !props.pageData.loggedInUserDetails ?
                                                <div className="rightTopList">
                                                    <ul className="custmenuRight">
                                                        <li className={!props.mobileMenu ? `nav-item dropdown${state.style == "block" ? " active" : ""}` : `dropdown MobDropdownNav${state.style == "block" ? " active" : ""}`}  style={{cursor:"pointer"}}  >
                                                            <a className={!props.mobileMenu ? "parent nav-link notclose usepicHead logged-in-cnt" : "parent loggedUer notclose usepicHead logged-in-cnt"} onClick={(e) => openToggle("settings",e)}  style={{cursor:"pointer"}} href="#"
                                                                role="button" aria-labelledby={Translate(props, "Please login to continue")}>
                                                                    ...
                                                                    props.redirectLogin ?
                                                                        <li>
                                                                            <Link href="/login" prefetch="false">
                                                                                <a className="dropdown-item iconmenu" id="loginFormPopup">{Translate(props, "Login")}</a>
                                                                            </Link>
                                                                        </li>
                                                                        :
                                                                        <li>
                                                                            <a className="dropdown-item iconmenu" id="loginFormPopup" data-bs-toggle="modal" data-bs-target="#loginpop" href="/login">{Translate(props, "Login")}</a>
                                                                        </li>
                                                                    : null
                                                            }
                                                            {
                                                                    ...
                                                                    props.redirectLogin ?
                                                                        <li>
                                                                            <Link href="/signup" prefetch="false">
                                                                                <a className="dropdown-item iconmenu">{Translate(props, "Sign Up")}</a>
                                                                            </Link>
                                                                        </li>
                                                                        :
                                                                        <li>
                                                                            <a className="dropdown-item iconmenu" data-bs-toggle="modal" data-bs-target="#registerpop" href="/signup">{Translate(props, "Sign Up")}</a>
                                                                        </li>
                                                                    : null
                                                            }
                                        ...
                                        <div className="rightTopList">
                                            <ul className="custmenuRight">
                                            {
                                                    props.pageData.loggedInUserDetails ?
                                                        <React.Fragment>
                                                            <CreateButtons type="website" {...props} style={state.type == "createbuttons" ? state.style : "none"} openToggle={openToggle} setCreateButtonsWrapperRef={setCreateButtonsWrapperRef} />
                                                            {
                                                                props.pageData.levelPermissions && parseInt(props.pageData.levelPermissions["member.allow_messages"]) == 1 ?
                                                                    <li className="nav-item dropdown main">
                                                                        <Link href="/messanger" as="/messages" prefetch="false">
                                                            ...
                                                            <Notifications type="website" {...props} loading={state.loading} notifications={state.notifications} deleteNotification={deleteNotification} markUnread={markUnread} pagging={state.pagging} loadMoreContent={loadMoreContent} unread={state.unread} style={state.type == "notifications" ? state.style : "none"} openToggle={openToggle} setNotificationWrapperRef={setNotificationWrapperRef} />
                                                            <SettingMenus type="website"    {...props} style={state.type == "settings" ? state.style : "none"} openToggle={openToggle} setSettingsWrapperRef={setSettingsWrapperRef} />
                                                        </React.Fragment>
                                                        : null
                                                }
                                ...
                                <div className="MobmenuWrap">
                                    <nav className="navbar navbar-toggleable-lg">
                                        <div className="align-items-center d-flex">
                                            <div className="backArrow ms-2" onClick={(e) => {
                                                Router.back();
                                            }}>
                                                <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 0 24 24" width="24px" fill="var(--arowBackColor)"><path d="M0 0h24v24H0V0z" fill="none" opacity=".87"/><path d="M17.51 3.87L15.73 2.1 5.84 12l9.9 9.9 1.77-1.77L9.38 12l8.13-8.13z"/></svg>
                                            </div>
                                            <button className="navbar-toggler navbar-toggler-right menu-bar" onClick={openMobileMenu}>
                                        ...
                                        <div className={`collapse navbar-collapse bg-inverse MobMenuScroll${state.menuStyle}`}>
                                            {
                                                !props.pageData.loggedInUserDetails ?
                                                        ...
                                                        <ul>
                                                            {
                                                                !props.loginButtonHide ?
                                                                    props.redirectLogin ?
                                                                        <li>
                                                                            <Link href="/login" prefetch="false">
                                                                                <a className="btncomm" id="loginFormPopup">{Translate(props, "Login")}</a>
                                                                            </Link>
                                                                        </li>
                                                                        :
                                                                        <li>
                                                                            <a className="btncomm" id="loginFormPopup" data-bs-toggle="modal" data-bs-target="#loginpop" href="/login">{Translate(props, "Login")}</a>
                                                                        </li>
                                                                    : null
                                                            }
                                                            {
                                                                !props.signButtonHide && props.pageData.appSettings['member_registeration'] == 1 ?
                                                                    props.redirectLogin ?
                                                                        <li>
                                                                            <Link href="/signup" prefetch="false">
                                                                                <a className="btncomm">{Translate(props, "Sign Up")}</a>
                                                                            </Link>
                                                                        </li>
                                                                        :
                                                                        <li>
                                                                            <a className="btncomm" data-bs-toggle="modal" data-bs-target="#registerpop" href="/signup">{Translate(props, "Sign Up")}</a>
                                                                        </li>
                                                                    : null
                                                            }
                                        ...
                                        <React.Fragment>
                                            ...
                                            <div className={`Mobsearch-bar collapse MobSearchbox-loggedout`} id="MobSearchbox">
                                                <form action="#" className="MobsearchForm">
                                                    <input className="form-control" id="search-text" type="text" value={state.search} onChange={(e) => { setState({ search: e.target.value }) }} />
                                                    <button onClick={search.bind(this)} className="btn btn-default search-btn">
                                                    <span className="material-icons" data-icon="search"></span>
                                                    </button>
                                                </form>
                                            </div>
                            ...
                            <React.Fragment>
                                <div className="MobRightSide headerRightMenu">
                                    <ul className="mobRightNav ">
                                        ...
                                        <Notifications {...props} type="mobile" loading={state.loading} notifications={state.notifications} deleteNotification={deleteNotification} markUnread={markUnread} pagging={state.pagging} loadMoreContent={loadMoreContent} unread={state.unread} style={state.type == "notifications" ? state.style : "none"} openToggle={openToggle} setNotificationWrapperRef={setNotificationWrapperRef} />
                                        <CreateButtons  {...props} type="mobile" style={state.type == "createbuttons" ? state.style : "none"} openToggle={openToggle} setCreateButtonsWrapperRef={setCreateButtonsWrapperRef} />
                                        {
                                            props.pageData.levelPermissions && parseInt(props.pageData.levelPermissions["member.allow_messages"]) == 1 ?
                                            <li className="dropdown main">
                                                <Link href="/messanger" as="/messages" prefetch="false">
                                        ...
                                        <SettingMenus {...props} type="mobile" style={state.type == "settings" ? state.style : "none"} openToggle={openToggle} setSettingsWrapperRef={setSettingsWrapperRef} />
                                    </ul>
                                </div>
                                {
                                    state.type == "notifications" ?
                                        <div className='mobMenuBox'>
                                            <ul>
                                                <NotificationItems type="mobile"  {...props} loadMoreContent={loadMoreContent} deleteNotification={deleteNotification} markUnread={markUnread} pagging={state.pagging} unread={state.unread} notifications={state.notifications} loading={state.loading} />
                                            </ul>
                                        </div>
                                        : null
                                }
