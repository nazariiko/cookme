...
const Comment = (props) => {
    ...
    const changeImage = (comment_id, picture) => {
        ...
        if (picture.target.files && picture.target.files[0] && (ext === "png" || ext === "jpeg" || ext === "jpg" || ext === 'PNG' || ext === 'JPEG' || ext === 'JPG' || ext === 'gif' || ext === 'GIF')) {
            if (!comment_id)
                setState({ commentImage: picture.target.files[0], commentImageDelete: true });
            else
                setState({ replyImage: picture.target.files[0], replyImageDelete: true });
        } else {
            if (!comment_id)
    ...
    const changeEditImage = (comment_id, picture) => {
        ...
        var ext = url.substring(url.lastIndexOf('.') + 1).toLowerCase();
        if (picture.target.files && picture.target.files[0] && (ext === "png" || ext === "jpeg" || ext === "jpg" || ext === 'PNG' || ext === 'JPEG' || ext === 'JPG' || ext === 'gif' || ext === 'GIF')) {
            setState({ editImage: picture.target.files[0], removeEditImage: true });
        } else {
            setState({ editImage: null, removeEditImage: true });
        }
    }
