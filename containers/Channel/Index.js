...
useEffect(() => {
    ...
    if (!state.needSubscription) {
      props.socket.on("videoAdded", (socketdata) => {
        if (
          ...
          props.openToast({message:Translate(props, socketdata.message), type:"success"});
          setTimeout(() => {
    ...
    props.socket.on("channelCoverReposition", (socketdata) => {
      ...
      if (stateRef.current && id == stateRef.current.channel_id) {
        ...
        setState(
          {  channel: item, loadingCover: false }
        );
        props.openToast(
          {
    ...
    axios
      ...
      .then((response) => {
        if (!response.data.error) {
          ...
          setState({openImportPopup:false,channelFormSubmit:false,channelID:""})
        } else {
          setState({
                          ...
                          >
                            <div className="details-tab-box">
                              <Members
                                ...
                                globalSearch={true}
                                channel_members={
                                  state.channel.supporters.results
                                }
                                channel_pagging={
                                  state.channel.supporters.pagging
                                }
                                channel_id={state.channel.channel_id}
                              />
                            </div>
