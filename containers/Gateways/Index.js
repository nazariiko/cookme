...
const Index = (props) => {
  ...
  const cashfree = () => {
    window.location.href =
      props.gatewaysUrl +
      (props.gatewaysUrl.indexOf("?") > -1 ? "&type=paypal" : "?type=cashfree");
  };
  const bank = () => {
  ...
  useEffect(() => {
    ...
    if (state.wallet) {
      countEnableGateways++;
      enableGateway = "wallet";
    }
    if (props.pageData.fromAPP && props.pageData.fromAppDevice == "ios") {
      let url = props.payPalURL
        ? props.payPalURL
        : props.gatewaysUrl +
          (props.gatewaysUrl.indexOf("?") > -1
            ? "&type=paypal"
            : "?type=paypal");
      let props = { ...props };
      props.purchaseData = {
        ...
        <div className="popup_cnt">
          <div className="comments">
            <div className="payment-cnt">
              ...
              !state.wallet &&
              (!state.packageObj ||
                (state.packageObj && !state.packageObj.apple_id)) ? (
                ...
                <div className=" gateway-options">
                  ...
                  props.pageData.fromAppDevice == "ios" ? (
                    <a
                      href={`/applepay?password=${props.pageData.appSettings.apple_inapp_purchasesecret}&user_id=${props.pageData.loggedInUserDetails.user_id}&price=${state.packageObj.price}&apple_id=${state.packageObj.apple_id}&package_id=${state.packageObj.package_id}&recurring=${state.packageObj.is_recurring}`}
                      className="apple_pay"
                    >
