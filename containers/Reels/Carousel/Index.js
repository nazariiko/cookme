...
const Carousel = (props) => {
  ...
  let content = state.items.map((item, index) => {
    ...
    if (typeof CloudFrontUrl === 'undefined') {
      ImageUrl = props.pageData.imageSuffix + item.image;
       // console.error('CloudFrontUrl is undefined using original URL', ImageUrl);
    } else {
      ImageUrl = CloudFrontUrl + item.image + '?format=auto&width=640/340';
      // console.log('CloudFrontUrl', ImageUrl);
    }
    return (
          ...
          >
            <a className="storyThumb-content storyThumb-overlay">
              ...
              <div className="storyThumb-profileImg reelThumb-profileImg">
                <img src={props.pageData.imageSuffix + item.avtar} alt={item.title} />
