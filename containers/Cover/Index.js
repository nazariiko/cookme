...
const Cover = (props) => {
    ...
    useEffect(() => {
        if(props.item.showCoverReposition){
            setState({item: props.item, reposition:true ,loadingCover:false ,loadingImage:false ,processing:false, percentCompleted:0 })
        }else if (props.item != state.item) {
          setState({ item: props.item ,loadingCover:false ,loadingImage:false ,processing:false, percentCompleted:0 })
        }else{
            setState({loadingCover:false,loadingImage:false })
        }
        if(props.item.showCoverReposition && props.item.showCoverReposition != state.item.showCoverReposition ){
            if(typeof $ != "undefined"){
        ...
        axios.post(url, formData, config)
            .then(response => {
                if (response.data.error) {
                    swal("Error", Translate(props, response.data.error[0].message), "error");
                    setState({percentCompleted:0,processing:false})
                }else{
                    //setState({percentCompleted:0,processing:true})
                }
            }).catch(err => {
                setState({ loadingCover: false,percentCompleted:0,processing:false })
            });
    }
    ...
    const openReport = (e) => {
        ...
        if (props.pageData && !props.pageData.loggedInUserDetails) {
            ...
            props.openReport({status:true, id:state.item.custom_url, type:'channels'})
        }
    }
    ...
    const repositionSave = (e) => {
        ...
        return (
            <React.Fragment>
                <div className={`${props.pageData.themeType != 2 ? "header-info-wrap container" : "header-info-wrap subscription-urser"} `}>
                            ...
                            {/* <div className="box-overlay"></div> */}
                                {
                                        ...
                                        <React.Fragment>
                                            <div className="editCoverImg">
                                                <a className="link" href={void(0)} title={Translate(props, "Edit cover photo")} onClick={e => {
                                                    coverref.current.click();
                                                }}>
                                                    <span className="material-icons" data-icon="camera_alt"></span>
                                                    <input className="fileNone" accept="image/*" onChange={uploadCover.bind()} ref={coverref} type="file" />
                                                </a>
                                            </div>
                                            {
                                                state.repositionDisplay == "block" && state.item.channelcover && !gif ? 
                                            <div className="editCoverImg resizeCoverImg">
                                                <a href="#" title={Translate(props, "Reposition cover photo")} onClick={repositionCover}>
                                                    ...
                                                    <div className="upload-progressbar">
                                                        {
                                                            ...
                                                            <React.Fragment>
                                                                ...
                                                                <div className="progressbar-cnt">
                                                                    <div className="progressbar" style={{width:state.percentCompleted+"%"}}></div>
                                                                </div>
                                                            </React.Fragment>
                                                        ...
                                                        <div className="imageprocess">
                                                            ...
                                                            {
                                                                props.t("Image is processing, this may take few minutes.")
                                                            }
                                                        </div>
                                                        : null
                                                        }
                                    ...
                                    <React.Fragment>
                                        ...
                                        <div className="editCoverImg">
                                            <a href="#" title={Translate(props, "Cancel reposition")} onClick={repositionCancel}>
                                            <span className="material-icons" data-icon="clear"></span>
                                            </a>
                                        </div>
                                        <div className="editCoverImg resizeCoverImg">
                                            <a href="#" title={Translate(props, "Save reposition")} onClick={repositionSave}>
                                ...
                                <div className="userInfo-block-content">
                                    <div className="userinfoLeft">
                                        <div className="userphoto-profile-img">
                                            ...
                                            {
                                                state.item.canEdit ?
                                                    <a className="editProfImg link" href={void(0)} title={Translate(props, "Edit profile photo")} onClick={e => {
                                                        imageref.current.click();
                                                    }}>
                                                        <span className="material-icons" data-icon="edit"></span>
                                                        <input className="fileNone" accept="image/*" onChange={uploadImage.bind()} ref={imageref} type="file" />
                                                    </a>
                                                    : null
                                            }
                                        ...
                                        <div className="user-profile-title">
                                            <h4><CensorWord {...props} text={state.item.title} />{ " "} {
                                                state.item.verified ?
                                            ...
                                            <div className="ChannelMoreinfo">
                                                {
                                                    props.pageData.appSettings['channel_featured'] == 1 && state.item.is_featured == 1 ?
                                                ...
                                                {
                                                    props.pageData.appSettings['channel_sponsored'] == 1 && state.item.is_sponsored == 1 ?
                                                ...
                                                {
                                                    props.pageData.appSettings['channel_hot'] == 1 && state.item.is_hot == 1 ?
                                            ...
                                            <div className="actionbtn" style={{display:"none"}}>
                                                {`${ShortNumber(state.item.view_count ? state.item.view_count : 0)}`}{" "}{props.t("view_count", { count: state.item.view_count ? state.item.view_count : 0 })}
                                            </div>
                                        </div>
                                    </div>
                                       ...
                                       <div className="follow-cnt">
                                       {
                                        state.item.approve == 1 ? 
                                            <div className="channel_subscribe">
                                                {
                                                    props.pageData.appSettings['channel_support'] == 1 && parseFloat(state.item.channel_subscription_amount) > 0 ? 
                                                        <Support {...props} item={state.item} item_type="channel" item_id={state.item.channel_id} />
                                                    :null
                                                }
                                                <Subscribe  button={true} fromView={true} {...props} title={state.item.follower_id ? props.t("Subscribed") : props.t("Subscribe")}  follow_count={state.item.follow_count} user={state.item} type={"channels"} id={props.id} />
                                                {`${ShortNumber(props.follow_count ? props.follow_count : 0)}`}{" "}{props.t("subscribe_count", { count: state.item.follow_count })}
                                            </div>
                                        : null 
                                        }
                                       </div>
                                    ...
                                    <ul className="LikeDislikeList">
                                        {
                                            props.pageData.appSettings[`${"channel_like"}`] == 1 && state.item.approve == 1 ?
                                                <li>
                                                    <Like {...props} icon={true} like_count={state.item.like_count} item={state.item} type={state.type} id={props.id} />
                                                </li>
                                                : null
                                        }
                                        {
                                            props.pageData.appSettings[`${"channel_dislike"}`] == 1 && state.item.approve == 1 ?
                                                <li>
                                                    <Dislike {...props} icon={true} dislike_count={state.item.dislike_count} item={state.item} type={state.type} id={props.id} />
                                                </li>
                                                : null
                                        }
                                        
                                        {
                                            props.pageData.appSettings[`${"channel_favourite"}`] == 1 && state.item.approve == 1 ?
                                                <li>
                                                    <Favourite {...props} icon={true} favourite_count={state.item.favourite_count} item={state.item} type={state.type} id={props.id} />
                                                </li>
                                                : null
                                        }
                                        ...
                                        {
                                            state.item.approve == 1 ?
                                                <SocialShare {...props} hideTitle={true} tags={state.item.tags} url={`${props.url}`} title={CensorWord("fn",props,state.item.title)} imageSuffix={props.pageData.imageSuffix} media={state.item.image} />
                                            : null
                                        }
                                        <li>
                                            <div className="dropdown TitleRightDropdown">
                                                    ...
                                                    
                                                {
                                                    state.item.canEdit ?
                                                    <li>
                                                        <Link href="/create-channel" customParam={`id=${state.item.custom_url}`} as={`/create-channel/${state.item.custom_url}`}>
                                                            <a href={`/create-channel/${state.item.custom_url}`}><span className="material-icons" data-icon="edit"></span>{Translate(props, "Edit")}</a>
                                                        </Link>
                                                        </li>
                                                        : null
                                                }
                                                {
                                                    ...
                                                    <li>
                                                        <a onClick={props.deleteChannel} href="#"><span className="material-icons" data-icon="delete"></span>{Translate(props, "Delete")}</a>
                                                        </li>
                                                        : null
                                                }
                                                {
            ...
            }
                {
                        props.pageData.appSettings['below_cover'] ? 
                            <AdsIndex paddingTop="20px" className="below_cover" ads={props.pageData.appSettings['below_cover']} />
                        : null
                    }
            </React.Fragment>
        )
